...
FilterHeadersStatus AddHeaderContext::onRequestHeaders(uint32_t) {
  ...
  auto path_str = std::string(path->view());
  logError(std::string("onRequestHeaders ") + path_str);
  if (path_str.compare("/vuln") == 0) {
    replaceRequestHeader(":path", "/health");
  }
...
FilterHeadersStatus AddHeaderContext::onResponseHeaders(uint32_t) {
  ...
  addResponseHeader("hello", "adela 7");
  return FilterHeadersStatus::Continue;
}
